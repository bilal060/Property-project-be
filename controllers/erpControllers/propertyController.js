...
const propertyController = {
    readProperty: async (req, res) => {
        try {
            // Find document by id
            const result = await PropertyModel.findOne({ _id: req.params.id, removed: false });
    ...
    , createProperty: async (req, res) => {
        try {
            ...
            console.log(body)
            const result = await new PropertyModel(body).save();
            console.log(body)
    ...
    updateProperty: async (req, res) => {
        try {
            ...
            const result = await PropertyModel.findOneAndUpdate({ _id: req.params.id, removed: false },
                body, {
    ...
    deleteProperty: async (req, res) => {
        try {
            ...
            // Find the document by id and delete it
            const result = await PropertyModel.findOneAndUpdate(
    ...
    listProperty: async (req, res) => {
        const { country, state, city, featured, user } = req.query
        ...
        try {
            ...
            const resultsPromise = PropertyModel.find(query)
                .skip(skip)
                .limit(limit)
                .sort({ created: 'desc' })
                .populate();
            // Counting the total documents
            const countPromise = PropertyModel.count(query);
    ...
    searchProperty: async (req, res) => {
        ...
        try {
            let results = await PropertyModel.find(fields).where('removed', false).limit(10);
            if (results.length >= 1) {
                return res.status(200).json({
                    success: true,
                    result: results,
    ...
    filterProperty: async (req, res) => {
        try {
            ...
            const result = await PropertyModel.find({ removed: false })
    ...
    , getFilterbyDateProperty: async (req, res) => {
        try {
